...
https://github.com/dippykit/dippykit/blob/master/LICENSE.txt
Authors: Brighton Ancelin, Motaz Alfarraj, Ghassan AlRegib

dippykit is licensed under the GNU General Public License v3.0
...
__all__ = ['huffman_encode', 'huffman_decode', 'huffman_dict', 'huffman_tree', 'get_dict_memory_usage']

def get_dict_memory_usage(dictionary):
    total_size = 0
    for key, value in dictionary.items():
        # Add size of key (fixed value assuming int32)
        total_size += 32
        # Add size of boolean array (fixed value assuming 1 bit per element)
...
def huffman_encode(
    ...
    tree_size = get_dict_memory_usage(symbol_code_dict)
    return np.packbits(im_enco), im_enco.size, symbol_code_dict, \
        symbol_probability_dict, tree_size
